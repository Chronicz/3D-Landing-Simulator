 ...
 //     assert(min < max);
	...
	//
	 bool overlap(const Box &box) {
		// grab min and max of both AABBs
		const Vector3 & minA = parameters[0];
		const Vector3 & maxA = parameters[1];
		const Vector3 & minB = box.parameters[0];
		const Vector3 & maxB = box.parameters[1];

		// Check for any separation along x, y, z axes
		...
		return overlapX && overlapY && overlapZ; // if all 3 axes overlap, boxes overlap
	}
